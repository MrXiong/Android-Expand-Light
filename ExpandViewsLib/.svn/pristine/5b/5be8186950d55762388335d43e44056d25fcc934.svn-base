...
public class TwoColumnView extends LinearLayout implements ViewBaseAction {
	...
	public Model setDefaultSelected(String modelName) {
		List<Model> nodes = mModel;

		for (int i = 0; i < nodes.size(); i++) {
			if (nodes.get(i).getModelName().equals(modelName)) {
				setDefaultSelect(i, TwoColumnView.NO_DEFAULT_SELECTED);
				return nodes.get(i);
			}
	...
	private void setDefaultSelect(final int selectedParentId, final int selectedChildId) {
