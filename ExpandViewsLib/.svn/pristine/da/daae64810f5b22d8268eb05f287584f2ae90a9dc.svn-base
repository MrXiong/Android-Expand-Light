...
public class ExpandTabView extends LinearLayout implements OnDismissListener {
	...
	public void updateExpandTabView(ArrayList<View> filterConditionViews,
			View selectConditionView, final String selectText) {

		onPressBack();
		int position = 0;
		for (int i = 0, len = filterConditionViews.size(); i < len; i++) {
			if (filterConditionViews.get(i) == selectConditionView) {
				position = i;
				break;
			}
		}
		setTitle(selectText, position);
